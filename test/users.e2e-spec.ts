...
suite('Users management integration tests', () => {
  ...
  suite('Create', () => {
    ...
    test('Invalid - bad body rejected (422)', async () => {
      const bodies = [
        { lastName: 'User', email: `e2e-${randomString(8)}@x.io` },
        { firstName: '', lastName: 'User', email: `e2e-${randomString(8)}@x.io` },
        { firstName: 'T', lastName: 'User', email: 'not-an-email' },
      ];
      await Promise.all(
        bodies.map((body) => {
    ...
    test('Invalid - non-masteradmin forbidden', async () => {
      await validateForbidden({
        ...
        path: ADMIN_PATH,
        method: 'post',
        body: { firstName: 'T', lastName: 'U', email: `e2e-${randomString(8)}@x.io` },
      });
    });
